FROM gitlab/gitlab-ce:11.6.5-ce.0 as builder
...
RUN set -xe \
    ...
    && git diff --diff-filter=d v11.6.5..v11.6.5-zh -- . ${IGNORE_DIRS} > ../zh_CN.diff \
    && echo " # Patching ..." \
    && patch -d ${GITLAB_DIR} -p1 < ../zh_CN.diff \
    && echo " # Copy assets files ..." \
    && git checkout v11.6.5-zh \
...
#  Production Stage  #
######################
FROM gitlab/gitlab-ce:11.6.5-ce.0 as production
...
ENV GITLAB_VERSION=v11.6.5
ENV GITLAB_DIR=/opt/gitlab/embedded/service/gitlab-rails
ENV GITLAB_GIT_ZH=https://gitlab.com/caeret/gitlab-ce.git
ENV GITLAB_GIT_COMMIT_UPSTREAM=v11.6.5
ENV GITLAB_GIT_COMMIT_ZH=v11.6.5-zh
